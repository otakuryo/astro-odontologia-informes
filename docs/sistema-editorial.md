...
| `PrintToolbar` | Compone `SiteNavbar` con el join **Estilo visual** (`VISUAL_THEMES`: Normal / Rounded / Glassmorfismo), el join **Papel** (Carta / A5 / A6; no A4), Descargar PDF (`btn-primary`), Imprimir hoja (`btn-ghost`, `window.print()`) y Opciones. Monta `ExportPanel`. Visible solo en pantalla. No se usa en el catálogo. No duplicar este selector en una página de formato. |
