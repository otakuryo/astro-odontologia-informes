...
| Página | 215,9 × 279,4 mm |
| `@page` | `size: letter portrait; margin: 0` |
| Margen seguro | 12 mm en cada lado |
| Área útil | 191,9 × 255,4 mm |
| Encabezado clínico | 28 mm |
| Pie documental | 9 mm |
| Cuerpo | El resto del área útil (`minmax(0, 1fr)`), sin saltos internos |

Cada ruta de formato genera **una sola hoja**. No hay paginación clínica: el pie lleva `1/1`.

En pantalla la hoja blanca se centra sobre un fondo neutro (`.is-print-document`). Al imprimir desaparecen el catálogo, la barra de impresión y las sombras. El catálogo (`/`) no es un documento Carta: está marcado como `.is-catalog` y se oculta con `@media print`.
...
Definidos en `src/styles/tokens.css` (valores en milímetros salvo la holgura de pantalla).
...
| --- | --- |
| `src/styles/tokens.css` | Paleta, tipografía, escala, grosores y dimensiones del lienzo |
| `src/styles/base.css` | Caja, tipografía base y vista de pantalla (hoja centrada) |
| `src/styles/print.css` | `@page`, ocultación de cromo de pantalla y ajuste de color |
...
| --- | --- |
| `PrintDocumentLayout` | Documento HTML completo: fuentes, estilos, barra de impresión, hoja Carta, encabezado, `slot` clínico y pie. Props: `title` (pestaña), `documentTitle`, `code`, `revision` y `page` opcionales. |
| `ClinicalHeader` | Banda de 28 mm. Prop `title`; etiquetas de los cuatro campos opcionales, resueltas con `ManualField`. |
| `DocumentFooter` | Pie de 9 mm. Prop `code`; `revision` por defecto `REV. 01`; `page` por defecto `1/1`. |
...
| `PrintToolbar` | Botón `window.print()`, visible solo en pantalla. No se usa en el catálogo. |
