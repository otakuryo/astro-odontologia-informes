...
test.describe('Exportación PDF', () => {
  ...
  test('ODO-F05 captureSheet: el perfil FDI 1.1 tiene interior de papel, contorno de tinta y más contraste en el interior', async ({
    page,
  }) => {
    ...
    await page.goto('/formatos/periodontograma/?papel=a5');
    await page.locator('.sheet').waitFor();
    await page.locator('[data-testid="perio-tooth"][data-fdi="1.1"][data-row="profile"]').waitFor();
    await page.evaluate(() => document.fonts.ready);
    await page.waitForFunction(() => Boolean(window.__odoPrintExport));

    /* La misma PNG que composePdf() embebe; se valida el ráster, no el binario PDF. */
    const samples = await page.evaluate(async () => {
      const api = window.__odoPrintExport;
      const sheet = document.querySelector('.sheet');
      ...
      try {
        ...
        if (best.length < minRun) {
          throw new Error(
            `no hay interior de papel continuo en el perfil FDI 1.1 (racha ${best.length} px, mínimo ${minRun})`,
          );
        }

        return { interior: best.interior, outline: best.outline };
      } finally {
        bitmap.close();
      }
    });
    ...
    expect(
      ...
      `píxel interior del perfil FDI 1.1 debe ser papel claro ${JSON.stringify(samples.interior)}`,
    ).toBeGreaterThan(230);
    expect(
      samples.outline.luma,
      `borde del perfil FDI 1.1 debe ser tinta oscura ${JSON.stringify(samples.outline)}`,
    ).toBeLessThan(140);
    expect(
      samples.interior.luma,
      `el interior debe contrastar más que el contorno (interior ${samples.interior.luma.toFixed(1)} vs contorno ${samples.outline.luma.toFixed(1)})`,
    ).toBeGreaterThan(samples.outline.luma);
  });
});
